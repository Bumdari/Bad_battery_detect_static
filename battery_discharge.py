...
def analyze_strings(df_seg, active):
    ...
    for s in active:
        ...
        # Сүүлийн 20 discharge-ийн дундаж — одоогийн байдлыг тусгана
        recent_n  = min(20, len(caps[s]))
        avg_cap   = round(float(np.mean(caps[s][-recent_n:])),  1)
        avg_volt  = round(float(np.mean(volts[s][-recent_n:])), 2)
        results[s] = {'stopper_count': counts[s], 'stopper_pct': pct,
                      'avg_cap': avg_cap, 'avg_volt': avg_volt}
    return results, total_stops

def classify_string(pct, avg_cap, avg_volt, recent_med, other_results):
    """
    Статус — тухайн string-ийн ЗОГСООСОн % + cap + нөгөөтэй харьцуулалтаар.
    recent_med (site-ийн) зөвхөн BAD давхар нотолгоонд хэрэглэнэ.

    BAD  : pct ≥ 70%
           эсвэл pct ≥ 40% АНД site recent_med < 30мин
    WEAK : pct ≥ 40%  эсвэл  нөгөөгөөсөө cap 15%+ доогуур
    GOOD : pct ≤ 15%  АНД  avg_cap ≥ 82%  АНД  site recent_med ≥ 30мин
    FAIR : бусад
    """
    cap_gap    = (max(r['avg_cap'] for r in other_results) - avg_cap) if other_results else 0
    site_short = (recent_med is not None and recent_med < 30)

    if pct >= STOPPER_BAD:
        return 'BAD', f'{pct:.0f}% discharge-д систем зогсоосон → батерей солих'
    elif pct >= STOPPER_WEAK and site_short:
        return 'BAD', f'{pct:.0f}% зогсоосон + site median {recent_med:.0f}мин < 30мин → батерей солих'
    ...
    elif pct <= STOPPER_GOOD and avg_cap >= 82 and not site_short:
        return 'GOOD', f'Зөвхөн {pct:.0f}% тохиолдолд зогсоосон — сайн'
    elif site_short:
        return 'FAIR', f'{pct:.0f}% зогсоосон — site муу орчинд ажиллаж байна'
    else:
...
def analyze_file(fp):
    ...
    for s in active:
        ...
        status, note = classify_string(r['stopper_pct'], r['avg_cap'], r['avg_volt'], rec_med, others)
    ...
    print(f"   Батерей солисон : {rep_date or 'илрүүлэгдсэнгүй'}")
...
def build_excel(all_sites, out_path):
    ...
    for idx,s in enumerate(all_sites):
        ...
        sc(row,3,s['rep_date'] if s['rep_date'] else 'Илрүүлэгдсэнгүй',
